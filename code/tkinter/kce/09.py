...
def kce_loop():
    slave_address = 2
    kce = minimalmodbus.Instrument("/dev/ttyUSB0",slave_address)
    kce.serial.baudrate=9600
    kce.serial.bytesize=8
    kce.serial.parity=minimalmodbus.serial.PARITY_NONE
    kce.serial.stopbits=1
    kce.serial.timeout=0.05
    kce.serial.mode=minimalmodbus.MODE_RTU
    while(var["run"]):
        sleep(1)
        try:
            _pv = kce.read_register(0x80,1)
            _sp = kce.read_register(0x00,1)
            var["PV"].set(_pv)
            var["SP"].set(_sp)
